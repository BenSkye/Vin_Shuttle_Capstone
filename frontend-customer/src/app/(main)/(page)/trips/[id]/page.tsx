...
export default function TripDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const { data, isLoading, error } = useTripSocket(id);
...
function VehicleInfo({ trip }: { trip: Trip }) {
    return (
...
// Driver and Status Information Component
function TripStatusInfo({ trip }: { trip: Trip }) {
    return (
...
function ServiceDetails({ trip }: { trip: Trip }) {
...
function RatingSection({
    ...
    // Show rating form if trip is completed and not yet rated
    if (trip.status === TripStatus.COMPLETED && !trip.isRating && !ratingSubmitted) {
        return (
    ...
    // Show submitted rating if available
    if (trip.status === TripStatus.COMPLETED && (trip.isRating || ratingSubmitted) && trip.rating) {
        return (
            <div className="mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-gray-200">
