...
import { busStopService, BusStop } from "../../services/busServices";
...
// Thêm interface mở rộng từ BusStop để bao gồm thuộc tính color
export interface BusStopWithColor extends BusStop {
  color: string;
  position: L.LatLng; // Ghi đè position để sử dụng L.LatLng thay vì Position
}
    ...
    async (latlng: L.LatLng) => {
      ...
      try {
        ...
        const newBusStop: BusStopWithColor = {
          _id: `temp-${Date.now()}`, // ID tạm thời cho điểm dừng mới
          name: `Điểm dừng mới`,
          description: "",
          address,
          position: latlng,
    ...
    <div className="h-screen flex">
      ...
      <div className="w-80 bg-white shadow-lg p-4 overflow-y-auto">
        ...
        {!isCreatingBusStop && !isEditing && selectedBusStop && (
          <div className="mb-4">
            <Button type="default" block onClick={toggleViewMode}>
              {viewMode === "selected"
