...
export class BookingService implements IBookingService {
  ...
  async bookingHour(
    ...
    const { startPoint, date, startTime, durationMinutes, vehicleCategories, paymentMethod } = data;
    ...
    for (const vehicle of vehicleSelected) {
      const driverSchedule = await validSchedules.filter(
    ...
    if (paymentMethod === PaymentMethod.MOMO) {
      const paymentResult = await this.checkoutService.CheckoutBookingMomo(
  ...
  async bookingScenicRoute(
    ...
    const { startPoint, scenicRouteId, date, startTime, vehicleCategories, paymentMethod } = data;
  ...
  async bookingSharedItinerary(
    ...
    const sharedItineraryFinded =
      await this.sharedItineraryService.findBestItineraryForNewTrip(searchSharedItineraryDto);
    const ListTrip = [];
    let totalAmount = 0;
    ...
    if (sharedItineraryFinded) {
      ...
      const bookingStartTime = now.add(
        BOOKING_BUFFER_MINUTES + sharedItineraryFinded.durationToNewTripStart / 60,
        'minute',
      );
      const bookingEndTime = now.add(
        BOOKING_BUFFER_MINUTES + sharedItineraryFinded.durationToNewTripEnd / 60,
        'minute',
      );
      ...
      const vehicle = await this.vehicleRepository.getById(
        sharedItineraryFinded.SharedItineraryDocument.vehicleId.toString(),
      );
      ...
      const newSharedItinerary =
        await this.sharedItineraryService.createSharedItinerary(createSharedItineraryDto);
      ...
      await this.sharedItineraryService.updateSharedItinerary(
        newSharedItinerary._id.toString(),
    ...
    for (const tripId of listTripId) {
      ...
      if (tripUpdate.serviceType === ServiceType.BOOKING_SHARE) {
        if (tripUpdate.servicePayload.bookingShare.isSharedItineraryMain) {
          ...
          await this.sharedItineraryService.saveASharedItineraryFromRedisToDBByTripID(
            tripId.toString(),
          );
        }
      }
      ...
      const listDriverTrip = await this.tripService.getPersonalDriverTrip(
  ...
  async payBookingFail(bookingCode: number): Promise<void> {
    ...
    const listTripId = booking.trips;
    for (const tripId of listTripId) {
      await this.tripRepository.deleteTrip(tripId.toString());
    }
    await this.bookingRepository.deleteBooking(booking._id.toString());
  }
