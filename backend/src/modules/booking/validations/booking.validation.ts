...
const vehicleCategoryItemSchema = Joi.object({
    categoryVehicleId: Joi.string().required().messages({
        'string.empty': ValidationMessages.string.empty,
        'any.required': ValidationMessages.string.required,
...
export const BookingValidation = {
    ...
    bookingHour: Joi.object({
        ...
        startTime: Joi.string().required().regex(/^\d{2}:\d{2}$/).messages({
            'string.empty': ValidationMessages.string.empty,
            'any.required': ValidationMessages.string.required,
            'string.pattern.base': 'Thời gian phải có định dạng HH:MM',
        }).label('Thời gian bắt đầu'),

        durationMinutes: Joi.number().integer().min(15).required().messages({
            'number.base': ValidationMessages.number.base,
            'number.integer': ValidationMessages.number.integer,
        ...
        vehicleCategories: Joi.array().items(vehicleCategoryItemSchema).min(1).required().messages({
    ...
    bookingScenicRoute: Joi.object({
        ...
        scenicRouteId: Joi.string().required().messages({
            'string.empty': ValidationMessages.string.empty,
            'any.required': ValidationMessages.string.required,
    ...
    bookingDestination: Joi.object({
        ...
        durationEstimate: Joi.number().min(1).required().messages({
            'number.base': ValidationMessages.number.base,
            'number.min': ValidationMessages.number.min,
            'any.required': ValidationMessages.any.required,
        }).label('Thời gian ước tính (phút)'),

        distanceEstimate: Joi.number().min(0.1).required().messages({
            'number.base': ValidationMessages.number.base,
            'number.min': ValidationMessages.number.min,
            'any.required': ValidationMessages.any.required,
        }).label('Khoảng cách ước tính (km)'),

        vehicleCategories: vehicleCategoryDestinationSchema.required().messages({
            'any.required': ValidationMessages.any.required,
        }).label('Danh mục xe'),
    ...
    bookingSharedItinerary: Joi.object({
        ...
        numberOfSeat: Joi.number().integer().min(1).required().messages({
            'number.base': ValidationMessages.number.base,
            'number.integer': ValidationMessages.number.integer,
            'number.min': ValidationMessages.number.min,
            'any.required': ValidationMessages.any.required,
