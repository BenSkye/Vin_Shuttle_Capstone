...
import { ScenicRoute } from './scenic-route.schema';
...
export class ScenicRouteRepository implements IScenicRouteRepository {
    ...
    async create(route: ICreateScenicRouteDto): Promise<ScenicRoute> {
        const newScenicRoute = new this.routeModel(route);
        return await newScenicRoute.save();
    }

    async findById(id: string): Promise<ScenicRoute | null> {
        return await this.routeModel.findById(id);
    }

    async find(query: any): Promise<ScenicRoute | null> {
        return await this.routeModel.findOne(query);
    }

    async findAll(): Promise<ScenicRoute[]> {
        return await this.routeModel.find();
    }

    async update(id: string, route: IUpdateScenicRouteDto): Promise<ScenicRoute | null> {
